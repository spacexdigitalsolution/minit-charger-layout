...
export default function AltusEngineeringStrip({ data }) {
  const getIcon = (name) => {
    switch (name) {
      case 'shield':
        return (
    ...
    <section className="bg-ink-950 border-y border-white/10">
      <div className="container mx-auto max-w-[1180px] px-6">
        <div className="flex flex-wrap items-stretch">
          {data.map((item, idx) => (
            <div key={idx} className={`flex-1 min-w-[200px] p-[22px_24px] flex items-center gap-3.5 ${idx !== data.length - 1 ? 'md:border-r border-white/10' : ''} ${idx !== 0 ? 'border-t md:border-t-0 border-white/10' : ''}`}>
              {getIcon(item.icon)}
              <div>
